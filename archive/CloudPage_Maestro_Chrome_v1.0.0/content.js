            ...
            <div class="cpm-header-left">
                <div class="cpm-logo">
                    ...
                    <div class="cpm-logo-text">
                        ...
                        <span class="cpm-logo-sub">SFMC Asset Manager</span>
                    </div>
                </div>
            </div>
...
function addStyles() {
    ...
    style.textContent = `
        ...
        .cpm-logo-icon-wrap {
            ...
            flex-shrink: 0;
            box-shadow: 0 0 12px rgba(255,255,255,0.15), inset 0 1px 0 rgba(255,255,255,0.2);
        }
        .cpm-logo-img {
            width: 30px;
            height: 30px;
        ...
        #cloudpages-manager.cpm-dark .cpm-token-dot { background: #7ee787; }
        #cloudpages-manager.cpm-dark .cpm-token-badge.error .cpm-token-dot { background: #f85149; }
        ...
        #cloudpages-manager.cpm-dark .cpm-skeleton {
            background: linear-gradient(90deg, #21262d 25%, #2d333b 50%, #21262d 75%) !important;
...
async function enrichVisibleItems(appcoreToken) {
    ...
    for (const item of itemsToEnrich) {
        ...
        try {
            ...
            enrichedCount++;
            // Patch only this row — never re-render the full table during enrichment
...
async function fetchLandingPagesV2(stack, appcoreToken) {
    if (!stack || !appcoreToken) return [];
    const url = `https://cloud-pages.${stack}.marketingcloudapps.com/fuelapi/internal/v2/cloudpages/landing-pages?$page=1&$pageSize=500&$orderBy=modifiedDate%20DESC`;
    try {
        const data = await new Promise((resolve, reject) => {
            chrome.runtime.sendMessage({
                ...
                config: {
                    url,
                    method: 'GET',
                    headers: { 'Accept': 'application/json', 'X-CSRF-Token': appcoreToken }
                }
            }, (response) => {
...
async function fetchThumbnail(thumbnailAssetId) {
    return new Promise((resolve) => {
        chrome.runtime.sendMessage({ type: 'GET_TOKENS' }, (tokenResponse) => {
            if (!tokenResponse?.success || !tokenResponse.tokens.pageHookToken) { resolve(null); return; }
            const stack = window.CPM_STATE.stack;
            const ph = tokenResponse.tokens.pageHookToken;
            const url = `https://content-builder.${stack}.marketingcloudapps.com/fuelapi/asset/v1/content/assets/${thumbnailAssetId}/file`;
            chrome.runtime.sendMessage({
                type: 'MAKE_REQUEST',
                config: { url, method: 'GET', headers: { 'X-CSRF-Token': ph } }
            }, (response) => {
...
async function exportAllToCSV() {
    ...
    const tokenResponse = await new Promise(resolve => chrome.runtime.sendMessage({ type: 'GET_TOKENS' }, resolve));
    if (!tokenResponse?.success || !tokenResponse.tokens?.pageHookToken) {
        showNotification('Authentication required. Please refresh the page.', 'error');
        return;
    }
    const pageHookToken = tokenResponse.tokens.pageHookToken;
    ...
    const _dk = isDarkMode();
    const _cardBg   = _dk ? '#161b22'  : '#ffffff';
    const _titleCol = _dk ? '#e6edf3'  : '#032d60';
    const _subCol   = _dk ? '#8b949e'  : '#706e6b';
    const _trackBg  = _dk ? '#21262d'  : '#e5e5e5';
    const _cancelBd = _dk ? '#30363d'  : '#dddbda';
    const _cancelCl = _dk ? '#8b949e'  : '#706e6b';
    overlay.innerHTML = `
        <div style="background:${_cardBg};border-radius:6px;padding:28px 32px;min-width:340px;max-width:90%;box-shadow:0 8px 32px rgba(0,0,0,0.35);text-align:center;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
            <div style="font-size:15px;font-weight:600;color:${_titleCol};margin-bottom:6px;">Exporting All Assets</div>
            <div id="cpm-export-progress-text" style="font-size:13px;color:${_subCol};margin-bottom:16px;">Initializing...</div>
            <div style="background:${_trackBg};border-radius:4px;height:8px;overflow:hidden;margin-bottom:16px;">
            ...
            <button id="cpm-export-cancel" style="background:none;border:1px solid ${_cancelBd};border-radius:4px;padding:6px 16px;font-size:12px;color:${_cancelCl};cursor:pointer;">Cancel</button>
        </div>
    `;
    const panel = document.getElementById('cloudpages-manager');
    ...
    try {
        ...
        while (!cancelled) {
            ...
            const result = await new Promise((resolve, reject) => {
                chrome.runtime.sendMessage({
                    type: 'MAKE_REQUEST',
                    config: {
                        url: `https://content-builder.${stack}.marketingcloudapps.com/fuelapi/asset/v1/content/assets/query?scope=ours`,
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json;datekind=local',
        ...
        if (appcoreToken && landingPages.length > 0) {
            const batchSize = 10;
            let enriched = 0;
            for (let i = 0; i < landingPages.length && !cancelled; i += batchSize) {
                ...
                await Promise.all(batch.map(async (item) => {
                    try {
                        const url = `https://cloud-pages.${stack}.marketingcloudapps.com/fuelapi/internal/v2/cloudpages/sites?siteAssetId=${item.id}`;
                        const resp = await new Promise((resolve, reject) => {
                            chrome.runtime.sendMessage({
                                type: 'MAKE_REQUEST',
                                config: { url, method: 'GET', headers: { 'Accept': 'application/json', 'X-CSRF-Token': appcoreToken } }
                            }, (r) => {
                                if (chrome.runtime.lastError) { reject(new Error(chrome.runtime.lastError.message)); return; }
