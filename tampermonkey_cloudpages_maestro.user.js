...
// @version      1.0.0
// @description  Batch manage Salesforce Marketing Cloud CloudPages — publish, unpublish, filter, sort, and export with a single click.
...
(function() {
    ...
    const ICONS = {
        ...
        infoCircle: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 11V17"/><path d="M12 7.01L12.01 6.9989"/></svg>',
    ...
    function renderTable() {
        ...
        // Note: Enrichment is handled by enrichBatchOfCodeResources() after initial load
        // Removing inline enrichment to prevent double-enrichment and performance issues
    }
    ...
    function createUI() {
        ...
        container.innerHTML = `
            <style>
                ...
                .cpm-about-header-text h2 {
                    color: #ffffff;
                    font-size: 16px;
                    font-weight: 700;
                    margin: 0 0 2px;
                }
                .cpm-about-header-text p {
            ...
            <!-- About Modal -->
            <div class="cpm-about-overlay" id="cpm-about-overlay">
                <div class="cpm-about-modal">
                    <div class="cpm-about-header">
                        <div class="cpm-about-header-text">
                            <h2>CloudPage Maestro</h2>
                            <p>v1.0.0 &mdash; SFMC CloudPages Manager</p>
                        </div>
                    </div>
                    <div class="cpm-about-body">
                        <p>Built by <strong>Aldorino Rrushi</strong> &copy; 2026. An unofficial tool for batch-managing Salesforce Marketing Cloud CloudPages.</p>
                        <div class="cpm-about-links">
                            <a href="https://www.linkedin.com/in/aldorino-rrushi/" target="_blank" class="cpm-about-link">
                            ...
                            <a href="https://martech-maestro-folio-sroh.vercel.app/" target="_blank" class="cpm-about-link">
                            ...
                            <a href="https://github.com/MetalHacker01" target="_blank" class="cpm-about-link">
                        ...
                        <button class="cpm-about-close" id="cpm-about-close-btn">Close</button>
                    </div>
                </div>
            </div>
    ...
    async function fetchAllFolders(pageHookToken) {
        if (!stack) {
            console.error('[CloudPage Maestro] No stack detected');
            return null;
        }
        
        console.log('[CloudPage Maestro] Fetching all CloudPages folders...');
        ...
        try {
            ...
            while (true) {
                    ...
                    GM_xmlhttpRequest({
                        ...
                        onload: (gmResponse) => {
                            if (gmResponse.status >= 200 && gmResponse.status < 300) {
                                resolve(JSON.parse(gmResponse.responseText));
                            } else {
                                reject(new Error(`Folders API error: ${gmResponse.status}`));
                            }
                        },
                        onerror: reject
                    });
                ...
                console.log(`[CloudPage Maestro] Page ${currentPage}: Fetched ${items.length} folders (Total: ${totalCount})`);
            ...
            console.log(`[CloudPage Maestro] Fetched all ${allFolders.length} folders from ${currentPage} page(s)`);
            ...
            console.error('[CloudPage Maestro] Error fetching folders:', error);
            return null;
        }
    }
    ...
    function buildFolderTree(folders) {
        console.log('[CloudPage Maestro] Building folder tree...');
        
        if (!folders || !folders.items) {
            console.error('[CloudPage Maestro] No folders to build tree from');
            return null;
        }
        ...
        folders.items.forEach(folder => {
            ...
            if (folder.parentId === 0) {
                ...
                if (parent) {
                    ...
                    console.warn(`[CloudPage Maestro] Parent ${folder.parentId} not found for folder ${folder.id}`);
                }
            }
        });
        
        console.log('[CloudPage Maestro] Folder tree built - Total folders:', folderMap.size);
    ...
    async function performBatchMove(selectedIds, targetFolderId, pageHookToken, folderMap) {
        ...
        showToast(`Moving ${selectedIds.length} items to "${targetFolder.name}"...`);
                    ...
                    GM_xmlhttpRequest({
                        ...
                        onload: (gmResponse) => {
                            if (gmResponse.status >= 200 && gmResponse.status < 300) {
                                ...
                                reject(new Error(`Move API error: ${gmResponse.status}`));
                            }
                        },
                        onerror: reject
                    });
        ...
        if (failCount === 0) {
            showToast(`Successfully moved ${successCount} items to "${targetFolder.name}". Click Refresh to update.`);
        } else if (successCount === 0) {
            showToast(`Failed to move all ${failCount} items`);
        } else {
            showToast(`Moved ${successCount} items, ${failCount} failed. Click Refresh to update.`);
        }
    ...
    function setupEventListeners() {
        ...
        document.getElementById('cpm-about-btn').addEventListener('click', () => {
            document.getElementById('cpm-about-overlay').classList.add('open');
        });
        document.getElementById('cpm-about-close-btn').addEventListener('click', () => {
            document.getElementById('cpm-about-overlay').classList.remove('open');
        });
        document.getElementById('cpm-about-overlay').addEventListener('click', (e) => {
